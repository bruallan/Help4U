...
import {
  ...
  CustomMarketScatterTooltip,
  CustomProductScatterTooltip,
...
function InnerApp() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        try {
          ...
          errStr = d.error || d.message || res.statusText;
        } catch (e) {}
      ...
      setIsSyncing(false);
      alert(
        "Sincronização do Supabase concluída com sucesso:\n" + data.message,
      );
  ...
  const [scatterCategoryFilter, setScatterCategoryFilter] =
    useState<string>("Todas");

  // Clustering Variables / Settings
  const [frentesParam, setFrentesParam] = useState<Record<string, number>>({});
  const [thresholdDestino, setThresholdDestino] = useState<number>(0.3);
  ...
  const calculateActionPlan = useCallback(() => {
    ...
    const filterStartMs = filterStartDate
      ? new Date(filterStartDate + "T00:00:00").getTime()
      : null;
    const filterEndMs = filterEndDate
    ...
    const filteredRows = rawData.filter((row) => {
      ...
      if (
        ...
        row.client &&
        !selectedUnits.includes(row.client)
      )
        return false;
      return true;
    });
    ...
    for (const row of filteredRows) {
      ...
      if (!prodMap.has(pName)) {
        prodMap.set(pName, {
          ...
          ticketsTotais: new Set(),
          ticketsExclusivos: new Set(),
        });
      }
    ...
    const interimData = Array.from(prodMap.entries()).map(([produto, data]) => {
      ...
      const taxaDestino =
        data.ticketsTotais.size > 0
          ? data.ticketsExclusivos.size / data.ticketsTotais.size
          : 0;
    ...
    const finalPlan: ActionPlanData[] = interimData.map((d) => {
      let cluster =
        "3 - Análise Manual (Baixa Intenção, Baixo Giro, Alta Densidade)"; // Fallback based on logical combinations
      let acaoRecomendada = "-";

      // Rules explicitly from prompt
      if (
        d.taxaDestino >= thresholdDestino &&
        d.volumeTotal >= finalThresholdGiro
      ) {
        cluster = "1 - Tratores";
        ...
        d.taxaDestino < thresholdDestino &&
        d.densidadeLucro >= finalThresholdDensidade
      ) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      try {
        ...
        const filteredRows = rawData.filter((row) => {
          if (filterStartMs && row.dayDate.getTime() < filterStartMs)
            return false;
          if (filterEndMs && row.dayDate.getTime() > filterEndMs) return false;
          if (
            selectedUnits.length > 0 &&
        ...
        for (const row of sortedForTx) {
          ...
          const dateStr = `${row.dayDate.getFullYear()}-${String(row.dayDate.getMonth() + 1).padStart(2, "0")}-${String(row.dayDate.getDate()).padStart(2, "0")}`;

          if (
            rowClient !== currentTxClient ||
            rowTime - currentTxStartTime > 3000
          ) {
        ...
        for (let i = 0; i < filteredRows.length; i++) {
          ...
          if (!globalMinDay || row.dayDate.getTime() < globalMinDay.getTime())
            globalMinDay = row.dayDate;
          if (!globalMaxDay || row.dayDate.getTime() > globalMaxDay.getTime())
            globalMaxDay = row.dayDate;
          ...
          if (row.buyerId) {
            pData.buyers.set(
              row.buyerId,
              (pData.buyers.get(row.buyerId) || 0) + 1,
            );
          } else {
            pData.pixCount++;
          }
          ...
          const testMargemLiquida =
            row.salePrice - row.costPrice - itemDeduction;
          ...
          if (!dailyFinances.has(dateStr)) {
            dailyFinances.set(dateStr, {
              date: row.dayDate,
              dateStr: row.dayDate.toLocaleDateString("pt-BR", {
                day: "2-digit",
                month: "2-digit",
              }),
              volume: 0,
        ...
        if (processedCount === 0) {
          throw new Error(
            "Nenhum dado válido encontrado. Verifique se as datas estão na Coluna A e os nomes na Coluna N.",
          );
        }

        const newStats: ProductStats[] = [];
        const totalGlobalDays =
          globalMinDay && globalMaxDay
            ? Math.round(
                (globalMaxDay.getTime() - globalMinDay.getTime()) / 86400000,
              ) + 1
            : 30;
        ...
        for (const [name, data] of productMap.entries()) {
          ...
          for (const d of dates) {
            ...
            if (
              uniqueDays.length === 0 ||
              day.getTime() > uniqueDays[uniqueDays.length - 1].getTime()
            ) {
              uniqueDays.push(day);
            }
          }
          ...
          if (salesCount <= 5) {
            ...
            if (globalMinDay && globalMaxDay) {
              ...
              const edgeBefore = Math.round(
                (firstDay.getTime() - globalMinDay.getTime()) / 86400000,
              );
              const edgeAfter = Math.round(
                (globalMaxDay.getTime() - lastDay.getTime()) / 86400000,
              );
          ...
          const margin =
            data.totalSale > 0
              ? ((data.totalSale - data.totalCost) / data.totalSale) * 100
              : 0;

          // Velocidade Bruta
          const grossDays = Math.max(
            1,
            Math.round((maxDate.getTime() - minDate.getTime()) / 86400000),
          );
          const grossVelocity = volume / grossDays;

          const margemLiquidaProd =
            data.totalSale - data.totalCost - data.totalDeduction;
          const marginPercentProd =
            data.totalSale > 0 ? (margemLiquidaProd / data.totalSale) * 100 : 0;
        ...
        newStats.forEach((s) => {
          if (s.volume >= medianVolume && s.hhi > HHI_CUTOFF)
            s.status = "Alerta de Risco";
          else if (s.volume >= medianVolume && s.hhi <= HHI_CUTOFF)
            s.status = "Motor da Loja";
          else if (s.volume < medianVolume && s.hhi <= HHI_CUTOFF)
            s.status = "Cauda Longa";
        ...
        const sortedDays = Array.from(dailyFinances.values()).sort(
        ...
        const sortedDailyPerf = Array.from(dailyProdPerfMap.values()).sort(
          (a, b) => a.date.getTime() - b.date.getTime(),
        );
        ...
        const newScatterStats: MarketScatterStat[] = marketArray.map((m) => {
          ...
          const volumePercent =
            globalVolume > 0 ? (m.volume / globalVolume) * 100 : 0;
          const marginPercent =
            m.faturamento > 0 ? (margemLiquida / m.faturamento) * 100 : 0;
          return {
            name: m.name,
            volumePercent,
            marginPercent,
        ...
        const finalProdScatterStats: ProductScatterStat[] = tempProdStats.map(
          (p) => {
            ...
            return {
              ...
              volumePercent:
                medianVolume > 0 ? (p.volume / (medianVolume * 2)) * 100 : 0,
            };
          },
        );
        ...
        setError(
          err.message ||
            "Ocorreu um erro ao processar o arquivo no período selecionado.",
        );
        setStats([]);
        setFinancialStats([]);
      } finally {
  ...
  useEffect(() => {
    ...
    const getTaxRateForClient = (clientName: string) => {
      ...
      if (cLower.includes("rappi")) return 0.2;
      return 0; // Default or Outros is 0
    };

    const yearData = rawData.filter(
      (row) => row.date.getFullYear().toString() === mensalSelectedYear,
    );
    ...
    const formattedData = Array.from(dataByMonth.entries()).map(
      ([mIdx, mData]) => {
        return {
  ...
  const availableCategories = useMemo(() => {
    return [
      "Todas",
      ...Array.from(new Set(productScatterStats.map((p) => p.category))).sort(),
    ];
  }, [productScatterStats]);

  const filteredProductScatterStats = useMemo(() => {
    if (scatterCategoryFilter === "Todas") return productScatterStats;
    return productScatterStats.filter(
      (p) => p.category === scatterCategoryFilter,
    );
  ...
  const levelCurvesData = useMemo(() => {
    ...
    const volumes = productScatterStats
      .map((s) => s.volume)
      .sort((a, b) => a - b);
    const medianVolume = volumes[Math.floor(volumes.length / 2)] || 0;
  ...
  const handleZoomIn = () => {
    ...
    setXDomain((prev) => {
      ...
      return [
        ...
        Math.min(10000, center + newRange / 2),
      ];
    });
    setYDomain((prev) => {
      const range = prev[1] - prev[0];
      ...
      return [
        Math.max(0, center - newRange / 2),
        Math.min(maxVol, center + newRange / 2),
      ];
    });
  };
    ...
    <div className="flex h-screen bg-slate-50 dark:bg-slate-950 font-sans overflow-hidden transition-colors duration-300">
      ...
      >
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          <button
            onClick={() => {
              setActiveTab("dispersao_mercados");
              setIsSidebarOpen(false);
            }}
            className={cn(
              "w-full flex items-center space-x-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors",
          ...
          <button
            onClick={() => {
              setActiveTab("dispersao_produtos");
              setIsSidebarOpen(false);
            }}
            className={cn(
              "w-full flex items-center space-x-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors",
          ...
          <button
            onClick={() => {
              setActiveTab("desempenho_mensal");
              setIsSidebarOpen(false);
            }}
            className={cn(
              "w-full flex items-center space-x-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors",
            ...
            <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-emerald-600 dark:text-emerald-500 mt-1">
                    Processando dia {syncStatus.currentDay} de{" "}
                    {syncStatus.totalDays} ({syncStatus.currentDate})
                  </p>
                </div>
              </div>
              <div className="w-full sm:w-80 max-w-sm flex items-center space-x-4">
                <div className="flex-1">
                    ...
                    <span>
                      {Math.round(
                        (syncStatus.currentDay /
                          Math.max(1, syncStatus.totalDays)) *
                          100,
                      )}
                      %
                    </span>
                  ...
                  <div className="h-2 w-full bg-emerald-200 dark:bg-emerald-900/50 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.round((syncStatus.currentDay / Math.max(1, syncStatus.totalDays)) * 100)}%`,
                      }}
                    />
                  </div>
                </div>
                <button
                  onClick={async () => {
                    if (
                      window.confirm("Deseja realmente parar a sincronização?")
                    ) {
            ...
            <header>
              <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">
                {activeTab === "vendas"
                  ? "Dashboard de Vendas"
                  : activeTab === "lucro_fluxo"
                    ? "Lucro e Fluxo Diário"
                    : activeTab === "dispersao_mercados"
                      ? "Dispersão de Mercados"
                      : activeTab === "dispersao_produtos"
                        ? "Dispersão de Produtos"
                        : activeTab === "desempenho_tipo"
                          ? "Desempenho Tipo"
                          : activeTab === "desempenho_mensal"
                            ? "Desempenho Mensal"
 ...
 : "Veja alertas de risco para seus produtos."}
                    ...
                    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                      <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
                        ...
                        <button
                          ...
                          className="flex items-center space-x-2 px-3 py-1.5 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition"
                        >
                        ...
                        <table className="min-w-full divide-y divide-slate-200">
                          <thead className="bg-slate-50">
                            <tr>
                              ...
                              <th
                                scope="col"
                                className="px-6 py-4 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors"
                              ...
                              <th
                                scope="col"
                                className="px-6 py-4 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors"
                                onClick={() => handleSort("grossVelocity")}
                              >
                                Velocidade Bruta
                                <br />
                                <span className="text-[10px] font-medium normal-case text-slate-400">
                                ...
                                {getSortIcon("grossVelocity")}
                              </th>
                              <th
                                scope="col"
                                className="px-6 py-4 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors"
                                onClick={() => handleSort("timeToSellOne")}
                              >
                                Tempo para Vender 1 Unidade
                                <br />
                                <span className="text-[10px] font-medium normal-case text-slate-400">
                                  (dias)
                                </span>{" "}
                                {getSortIcon("timeToSellOne")}
                              </th>
                            </tr>
                          </thead>
                          <tbody className="bg-white divide-y divide-slate-100">
                            {filteredStats.length > 0 ? (
                              applySort<any>(filteredStats).map(
                                (stat: any, idx: number) => (
                                  <tr
                                    key={idx}
                                  ...
                                  >
                                        ...
                                        <div className="flex flex-col items-end">
                                          ...
                                          {stat.ruptureDays > 0 && (
                                            <span
                                              ...
                                              title={`${stat.ruptureDays.toFixed(1)} dias de ruptura ignorados`}
                                            >
                                              -{stat.ruptureDays.toFixed(1)}d
                                              ruptura
                                            </span>
                                          )}
                                          {stat.salesCount <= 5 && (
                                            <span
                                              ...
                                              title="Poucas vendas. Calculado com base em 30 dias."
                                            >
                                              Base 30 dias
                                            </span>
                                          )}
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-slate-600">
                                      {stat.timeToSellOne !== null ? (
                                        ...
                                        <span
                                          className="text-slate-400 text-xs"
                                          title="Dados insuficientes (apenas 1 venda ou vendas no mesmo dia)"
                                        >
                                          N/A
                                        </span>
                                      )}
                        ...
                        <div className="mb-6 flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                          <div>
                            <h3 className="text-lg font-semibold text-slate-900 dark:text-white">
                              Matriz de Risco de Demanda e Estoque
                            </h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                              Use os botões{" "}
                              <kbd className="px-1.5 py-0.5 bg-slate-200 dark:bg-slate-800 rounded-md text-xs font-mono font-semibold text-slate-700 dark:text-slate-300">
                                +
                              </kbd>{" "}
                              e{" "}
                              <kbd className="px-1.5 py-0.5 bg-slate-200 dark:bg-slate-800 rounded-md text-xs font-mono font-semibold text-slate-700 dark:text-slate-300">
                              ...
                              para dar zoom. Clique e arraste no gráfico para
                              percorrer os dados. Bolhas maiores indicam maior
                              margem. Bolhas{" "}
                              <span className="text-amber-500 font-medium">
                                laranjas
                              </span>{" "}
                              indicam alto índice de Venda Cega (PIX &gt; 40%).
                            </p>
                          </div>
                          <div className="flex items-center space-x-2">
                            <button
                      ...
                      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                        <div className="p-6 border-b border-slate-200">
                          <h3 className="text-lg font-semibold text-slate-900">
                            Itens em Alerta de Risco
                          </h3>
                          <p className="text-sm text-slate-500 mt-1">
                            Produtos com alto risco de encalhe (comprados por
                            poucas pessoas).
                          </p>
                        </div>
                        <div className="overflow-x-auto">
                          <table className="min-w-full divide-y divide-slate-200">
                            <thead className="bg-slate-50">
                              <tr>
                                ...
                                >
                                  Margem {getSortIcon("margin")}
                                </th>
                                <th
                                  scope="col"
                                  className="px-6 py-4 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors"
                                  onClick={() => handleSort("pixPercent")}
                                >
                                  Vendas PIX {getSortIcon("pixPercent")}
                                </th>
                              </tr>
                            </thead>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                          {[
                            ...
                            },
                            {
                              ...
                              value: financialStats.reduce(
                                (acc, curr) => acc + curr.margemLiquida,
                                0,
                              ),
                              type: "currency",
                            ...
                            <div
                              key={idx}
                              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm p-5 transition-colors"
                            >
                              <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">
                                {item.title}
                              </p>
                              <p
                                className={cn("text-2xl font-bold", item.color)}
                              >
                                {item.type === "currency"
                          ...
                          <ResponsiveContainer width="100%" height={500}>
                            ...
                            >
                              ...
                              <ReferenceLine
                                ...
                                y={
                                  financialStats.reduce(
                                    (sum, item) => sum + item.margemLiquida,
                                    0,
                                  ) / financialStats.length
                                }
                                stroke="#1e40af"
                      ...
                      {marketScatterStats.length > 0 ? (
                        <ResponsiveContainer width="100%" height={500}>
                          ...
                          >
                            ...
                            <RechartsTooltip
                              cursor={{ strokeDasharray: "3 3" }}
                              content={<CustomMarketScatterTooltip />}
                            />
                        ...
                        <div className="flex flex-col items-center justify-center py-12">
                          <AlertCircle className="w-10 h-10 text-slate-400 mb-4" />
                          <p className="text-slate-500 font-medium text-center">
                            Nenhum dado de mercado encontrado para criar o
                            gráfico.
                          </p>
                        </div>
                      )}
                      ...
                      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden p-6 transition-colors">
                        <div className="mb-6 border-b border-slate-100 dark:border-slate-800 pb-4">
                          <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-2">
                            Análise de Margem vs Volume
                          </h3>
                          <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed max-w-4xl">
                            Neste gráfico, cada ponto representa um produto
                            distinto. <br />
                            <strong>Entendendo o tamanho da bolinha:</strong> O
                            tamanho (diâmetro) do ponto é diretamente
                            proporcional ao{" "}
                            <strong>Volume Absoluto de Vendas</strong>. Uma
                            bolinha maior significa que o produto teve muitas
                            unidades vendidas no período. Bolinhas menores
                            representam produtos que saíram pouco. Isso ajuda a
                            identificar rapidamente os "campeões de venda"
                            (bolas grandes) e analisar se a margem deles está
                            saudável (posição no eixo vertical).
                          </p>
                          ...
                          <div className="mt-4 p-4 bg-orange-50 dark:bg-orange-900/20 border border-orange-100 dark:border-orange-800 rounded-xl text-sm text-orange-800 dark:text-orange-300">
                            ...
                            <p className="mb-2">
                              No gráfico abaixo, adicionamos curvas tracejadas
                              que representam as{" "}
                              <strong>
                                Zonas de Margem Líquida Total (R$)
                              </strong>
                              . Dois produtos podem gerar os mesmos R$ 50 de
                              lucro de formas diferentes: um com muito volume e
                              margem unitária pequena, outro com pouco volume e
                              margem unitária grande.
                            </p>
                            <div className="bg-white dark:bg-slate-950 p-3 rounded shadow-sm inline-block font-mono text-xs text-slate-700 dark:text-slate-300 mt-2 mb-2 w-full md:w-auto overflow-x-auto">
                              Equação da Margem Unitária:
                              <br />
                              <span className="font-bold">
                                M_unit = Preço_Venda × (1 - Repasse) - Custo
                              </span>
                              <br />
                              <br />
                              Fórmula das Curvas de Nível (Onde E = Espaçamento,
                              ex: 50, 100, 150):
                              <br />
                              <span className="font-bold">
                                Margem Unitária (Y) = E / Volume Absolute (X)
                              </span>
                            </div>
                            <p>
                              Isso cria o padrão de "escorregador" (uma
                              hipérbole). Quanto mais para a direita (Alto
                              Volume), menor a Margem Unitária necessária para
                              encostar na linha de R$ 100, por exemplo.
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="mb-4">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setScatterCategoryFilter(e.target.value)
                            }
                        ...
                        {filteredProductScatterStats.length > 0 ? (
                          <ResponsiveContainer width="100%" height={700}>
                            ...
                            >
                              ...
                              <RechartsTooltip
                                cursor={{ strokeDasharray: "3 3" }}
                                content={<CustomProductScatterTooltip />}
                              />
                              <Legend wrapperStyle={{ paddingTop: "20px" }} />
                              ...
                              {Array.from(
                                new Set(
                                  filteredProductScatterStats.map(
                                    (p) => p.category,
                                  ),
                                ),
                              )
                                ...
                                .map((cat, idx) => {
                                  ...
                                  const data =
                                    filteredProductScatterStats.filter(
                          ...
                          <div className="flex flex-col items-center justify-center py-12">
                            <AlertCircle className="w-10 h-10 text-slate-400 mb-4" />
                            <p className="text-slate-500 font-medium text-center">
                              Nenhum dado de produto encontrado para criar o
                              gráfico.
                            </p>
                          </div>
                        )}
                      ...
                      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden transition-colors">
                        <div className="p-6 border-b border-slate-200 dark:border-slate-800 bg-red-50/30 dark:bg-red-900/10">
                          ...
                          <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                            Abaixo estão os produtos cujo custo total é R$ 0,00
                            (seja por não ter sido cadastrado na planilha ou
                            ajustado automaticamente após o sistema identificar
                            que a margem líquida seria negativa em uma venda).
                          </p>
                        </div>
                        <div className="overflow-x-auto">
                          <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                            <thead className="bg-slate-50 dark:bg-slate-950">
                              <tr>
                                <th
                                  className="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors"
                                  onClick={() => handleSort("name")}
                                >
                                  Produto / Categoria {getSortIcon("name")}
                                </th>
                                <th
                                  className="px-6 py-4 text-right text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors"
                                  onClick={() => handleSort("volume")}
                                >
                                  Volume de Vendas {getSortIcon("volume")}
                                </th>
                                <th
                                  className="px-6 py-4 text-right text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors"
                                  onClick={() => handleSort("faturamento")}
                                >
                                  Faturamento Total {getSortIcon("faturamento")}
                                </th>
                              </tr>
                            </thead>
                        ...
                        {dailyProductPerformances.length > 0 ? (
                          <div className="flex flex-col xl:flex-row gap-6">
                            <div className="flex-1 h-[800px]">
                              <ResponsiveContainer width="100%" height="100%">
                                <LineChart
                                  data={dailyProductPerformances.map((day) => {
                                    let totalSelected = 0;
                                    desempenhoSelectedProducts.forEach((p) => {
                                      totalSelected += (day[p] as number) || 0;
                                    });
                                    return {
                                      ...day,
                                ...
                                >
                                      ...
                                      if (active && payload && payload.length) {
                                        return (
                                          <div className="bg-white dark:bg-slate-900 p-4 border border-slate-200 dark:border-slate-800 shadow-xl rounded-xl text-sm min-w-[200px]">
                                            <p className="font-bold text-slate-800 dark:text-slate-200 mb-2 border-b dark:border-slate-800 pb-2">
                                              {label}
                                            </p>
                                            <div className="space-y-1 text-slate-600 dark:text-slate-400">
                                              {payload.map(
                                                (entry: any, index: number) => (
                                                  <p
                                                    key={index}
                                                    className="flex justify-between items-center whitespace-nowrap gap-4"
                                                  >
                                  ...
                                  />
                                  {desempenhoSelectedProducts.map(
                                    (prod, idx) => (
                                      <Line
                                        key={prod}
                                        type="monotone"
                                  ...
                                  {desempenhoSelectedProducts.length > 0 && (
                                    <>
                                      ...
                                      <ReferenceLine
                                        y={(() => {
                                          const data =
                                            dailyProductPerformances.map(
                                              (day) => {
                                                let totalSelected = 0;
                                                desempenhoSelectedProducts.forEach(
                                                  (p) => {
                                                    totalSelected +=
                                                      (day[p] as number) || 0;
                                                  },
                            ...
                            <div className="w-full xl:w-80 flex flex-col bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 transition-colors">
                              <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 rounded-t-xl transition-colors">
                                ...
                                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                                  Soma das vendas nos dias filtrados
                                </p>
                              </div>
                                ...
                                {desempenhoSelectedProducts.length === 0 ? (
                                  ...
                                  <>
                                    <div className="flex justify-between items-center pb-3 mb-3 border-b border-slate-200 dark:border-slate-800">
                                      ...
                                      <span className="font-bold text-slate-900 dark:text-white">
                                        {desempenhoSelectedProducts.reduce(
                                          (acc, prod) => {
                                            return (
                                              acc +
                                              dailyProductPerformances.reduce(
                                                (dayAcc, day) =>
                                                  dayAcc + (day[prod] || 0),
                                                0,
                                              )
                          ...
                          <div className="flex flex-col items-center justify-center py-12">
                            ...
                            <p className="text-slate-500 dark:text-slate-400 font-medium text-center">
                              Selecione data e importe a planilha para ver a
                              análise de volume.
                            </p>
                          </div>
                        )}
                        ...
                        <div className="flex flex-col md:flex-row gap-4 mb-6 relative z-10">
                          <div className="flex-1">
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setMensalSelectedYear(e.target.value)
                              }
                            ...
                            >
                              {mensalAvailableYears.length === 0 && (
                                <option value={mensalSelectedYear}>
                                  {mensalSelectedYear}
                                </option>
                              )}
                              {mensalAvailableYears.map((y) => (
                          ...
                          <div className="flex-1">
                            ...
                            <select
                              value={mensalMetric}
                              onChange={(e) =>
                                setMensalMetric(e.target.value as any)
                              }
                              className="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block p-2.5 dark:bg-slate-950 dark:border-slate-800 dark:text-white"
                            >
                          ...
                          <div className="flex-[2]">
                            ...
                            <UnitDropdown
                              availableUnits={availableUnits}
                              selectedUnits={
                                mensalSelectedMarkets.length > 0
                                  ? mensalSelectedMarkets
                                  : availableUnits
                              }
                    ...
                    <div className="space-y-6">
                      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm p-6 overflow-hidden transition-colors">
                        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 space-y-4 md:space-y-0">
                          <div>
                            <h3 className="text-lg font-semibold text-slate-900 dark:text-white">
                              Motor de Clusterização de Sortimento
                            </h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                              Configure o espaço da gôndola e o algoritmo de
                              clusterização classificará os produtos em ações
                              estratégicas.
                            </p>
                            <p className="text-xs text-slate-400 dark:text-slate-500 mt-2">
                              Medianas (Auto): Volume ={" "}
                              <strong>{internalThresholds.giro} unid.</strong> |
                              Densidade ={" "}
                              <strong>
                                {new Intl.NumberFormat("pt-BR", {
                                  style: "currency",
                                  currency: "BRL",
                                }).format(internalThresholds.densidade)}
                              </strong>
                            </p>
                          </div>
                          <div className="flex items-center space-x-3 bg-slate-50 dark:bg-slate-950 p-2 rounded-xl border border-slate-200 dark:border-slate-800">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setThresholdDestino(Number(e.target.value))
                              }
                          ...
                          <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                            <thead className="bg-slate-50 dark:bg-slate-950">
                              <tr>
                                ...
                                <th
                                  className="px-6 py-4 text-center text-xs font-semibold text-orange-600 dark:text-orange-400 uppercase tracking-wider bg-orange-50/50 dark:bg-orange-900/10 cursor-pointer hover:bg-orange-100 transition-colors"
                                  onClick={() => handleSort("frentes")}
                                >
                                  Frentes Ocupadas {getSortIcon("frentes")}
                                </th>
                                <th
                                  className="px-6 py-4 text-right text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors"
                                  onClick={() => handleSort("volumeTotal")}
                                >
                                  Volume Total {getSortIcon("volumeTotal")}
                                </th>
                                <th
                                  className="px-6 py-4 text-right text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors"
                                  onClick={() => handleSort("densidadeLucro")}
                                >
                                  Densidade (R$) {getSortIcon("densidadeLucro")}
                                </th>
                                <th
                                  className="px-6 py-4 text-right text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors"
                                  onClick={() => handleSort("taxaDestino")}
                                >
                                  Tx. Destino {getSortIcon("taxaDestino")}
                                </th>
                                <th
                                  className="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors"
                                  onClick={() => handleSort("cluster")}
                                >
                                  Cluster Atribuído {getSortIcon("cluster")}
                                </th>
                                <th
                                  className="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:bg-slate-100 transition-colors"
                                  onClick={() => handleSort("acao")}
                                >
                                  Ação Recomendada {getSortIcon("acao")}
                                </th>
                              </tr>
                            </thead>
                            <tbody className="bg-white dark:bg-slate-900 divide-y divide-slate-200 dark:divide-slate-800">
                              {applySort<any>(
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <td className="px-6 py-4 text-sm text-right text-slate-600 dark:text-slate-400 font-medium">
                                      {row.volumeTotal}
                                    </td>
                                    <td className="px-6 py-4 text-sm text-right text-slate-600 dark:text-slate-400">
                                      {new Intl.NumberFormat("pt-BR", {
                                    ...
                                    <td className="px-6 py-4">
                                      <span
                                        className={cn(
                                          ...
                                          row.cluster.includes("Tratores") &&
                                            "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400",
                                          row.cluster.includes("Impulso") &&
                                            "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400",
                                          row.cluster.includes("Urgência") &&
                                            "bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400",
                                          row.cluster.includes(
                                            "Inadimplentes",
                                          ) &&
            ...
            {activeTab === "analise_cesta" && rawData && (
              <AnaliseCesta rawData={rawData} />
            )}
            {activeTab === "mapa_calor" && rawData && (
              <MapaCalor rawData={rawData} availableUnits={availableUnits} />
            )}
            {activeTab === "gestao_validade" && rawData && (
              <GestaoValidade
                rawData={rawData}
                availableUnits={availableUnits}
              />
