...
export function PosEstocagem() {
  ...
  const parseExcelFiles = async () => {
    ...
    try {
      ...
      const prodJson: any[][] = XLSX.utils.sheet_to_json(prodSheet, {
        header: 1,
      });
      ...
      for (let i = 0; i < Math.min(prodJson.length, 50); i++) {
        ...
        if (
          ...
          pHeaderRow = i;
          pCodCol = row.findIndex(
            (c) => c && String(c).toLowerCase().trim() === "código",
          );
          pNomeCol = row.findIndex(
            (c) => c && String(c).toLowerCase().trim() === "nome",
          );
          pCustoCol = row.findIndex(
            (c) => c && String(c).toLowerCase().includes("custo atual"),
          );
          if (pCustoCol === -1) {
            pCustoCol = row.findIndex(
              (c) => c && String(c).toLowerCase().includes("preço de custo"),
            );
          }
          if (pCustoCol === -1) {
            pCustoCol = row.findIndex(
              (c) => c && String(c).toLowerCase().includes("custo"),
            );
          }
          break;
        }
      ...
      for (const sheetName of planWorkbook.SheetNames) {
        ...
        const planJson: any[][] = XLSX.utils.sheet_to_json(sheet, {
          header: 1,
        });
      ...
      alert(
        "Houve um erro ao processar as planilhas. Verifique se o formato está correto.",
      );
    } finally {
  ...
  const missingProductsData: MissingProductRow[] = useMemo(() => {
    ...
    const activeMarkets = marketsList.filter((m) =>
      ...
      for (const prod of productsList) {
        if (!market.productCodes.has(prod.codigo)) {
          ...
          const suggestedPrice = Math.max(
            0,
            parseFloat(
              (Math.ceil(exactSuggestedPrice * 10) / 10 - 0.01).toFixed(2),
            ),
          );
          ...
          if (suggestedPrice > 0) {
            margem20 = 1 - cost / suggestedPrice - 0.2;
  ...
  const visibleProducts = useMemo(() => {
    return missingProductsData.filter(
      (row) => !ignoredItems.has(`${row.mercado}|${row.codigo}`),
    );
  ...
  const isAllMarketsSelected =
    selectedMarkets.size === marketsList.length && marketsList.length > 0;
    ...
    <div className="flex-1 grid grid-cols-1 gap-8 fade-in">
      <section className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
        <h2 className="text-xl font-semibold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
          <UploadCloud className="text-orange-500 w-5 h-5" /> Importar Planilhas
          de Análise
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="flex flex-col gap-2">
            ...
            <div className="relative group">
              ...
              <div
                className={cn(
                  "p-4 border-2 border-dashed rounded-xl flex items-center gap-3 transition-colors",
                  planogramasFile
                    ? "border-orange-500 bg-orange-50/50 dark:bg-orange-900/10"
                    : "border-slate-300 dark:border-slate-700 group-hover:border-orange-500 bg-slate-50 dark:bg-slate-800/50",
                )}
              >
                <FileSpreadsheet
                  className={
                ...
                <div className="flex-1 truncate">
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-300 truncate">
                    {planogramasFile
                      ? planogramasFile.name
                      : "Clique ou arraste Planogramas.xls"}
                  </p>
                </div>
              </div>
            </div>
              ...
              >
                ...
                <div className="flex-1 truncate">
                  <p className="text-sm font-medium text-slate-700 dark:text-slate-300 truncate">
                    {produtosFile
                      ? produtosFile.name
                      : "Clique ou arraste Produtos.xlsx"}
                  </p>
                </div>
              </div>
            </div>
      ...
      {marketsList.length > 0 && (
        <section className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col h-[700px]">
          {mappingInfo && (
            <div className="mx-6 mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 text-slate-800 dark:text-slate-200 border border-blue-200 dark:border-blue-800/50 rounded-xl text-sm flex gap-2 flex-col">
              <h3 className="font-semibold text-slate-900 dark:text-white border-b border-blue-200 dark:border-blue-800/50 pb-2 mb-1 flex items-center gap-2">
                ...
                />{" "}
                Diagnóstico de Leitura (Como Busquei os Dados)
              </h3>
              <p className="text-slate-600 dark:text-slate-400">
                Caso algum valor não bata, verifique se a coluna identificada
                está correta:
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-1">
                <div>
                  ...
                  <ul className="list-disc pl-5 mt-1 space-y-1 text-slate-600 dark:text-slate-400">
                    <li>
                      Cabeçalho:{" "}
                      <strong>Linha {mappingInfo.prodHeaderRow}</strong>
                    </li>
                    <li>
                      Código: <strong>Coluna {mappingInfo.prodCodCol}</strong>
                    </li>
                    <li>
                      Nome: <strong>Coluna {mappingInfo.prodNomeCol}</strong>
                    </li>
                    <li>
                      Preço de Custo:{" "}
                      <strong>Coluna {mappingInfo.prodCustoCol}</strong>
                    </li>
                  </ul>
                </div>
                <div>
                  <strong className="text-blue-700 dark:text-blue-400">
                    Planilha: Planogramas
                  </strong>
                  <ul className="list-disc pl-5 mt-1 space-y-1 text-slate-600 dark:text-slate-400">
                    <li>
                      Cabeçalho:{" "}
                      <strong>Linha {mappingInfo.planHeaderRow}</strong>
                    </li>
                    <li>
                      Código: <strong>Coluna {mappingInfo.planCodCol}</strong>
                    </li>
                  </ul>
                </div>
              </div>
                ...
                >
                  <span>
                    {isAllMarketsSelected
                      ? "Todos os mercados"
                      : `${selectedMarkets.size} mercado(s) selecionado(s)`}
                  </span>
          ...
          <div className="flex-1 overflow-auto bg-slate-50 dark:bg-slate-950/50 border-t border-slate-200 dark:border-slate-800">
            <table className="w-full text-left border-collapse text-sm">
              <thead className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 sticky top-0 z-10 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800">
                <tr>
                  <th
                    className="px-4 py-3 font-semibold cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                    onClick={() => handleSort("mercado")}
                  >
                    Mercado {getSortIcon("mercado")}
                  </th>
                  <th
                    className="px-4 py-3 font-semibold cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                  ...
                  <th
                    className="px-4 py-3 font-semibold w-1/4 cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                    onClick={() => handleSort("produto")}
                  >
                    Produto {getSortIcon("produto")}
                  </th>
                  <th
                    className="px-4 py-3 font-semibold text-right cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                    onClick={() => handleSort("precoCusto")}
                  >
                    Preço Custo {getSortIcon("precoCusto")}
                  </th>
                  <th
                    className="px-4 py-3 font-semibold text-right text-slate-900 dark:text-white bg-slate-200/50 dark:bg-slate-700/50 cursor-pointer hover:bg-slate-300/50 dark:hover:bg-slate-600/50 transition-colors"
                    onClick={() => handleSort("precoSugerido")}
                  >
                    Preço Sugerido {getSortIcon("precoSugerido")}
                  </th>
                  <th
                    className="px-4 py-3 font-semibold text-right cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                    onClick={() => handleSort("margem20")}
                  >
                    Mgm (20%) {getSortIcon("margem20")}
                  </th>
                  <th
                    className="px-4 py-3 font-semibold text-right cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                    onClick={() => handleSort("margem27")}
                  >
                    Mgm (27%) {getSortIcon("margem27")}
                  </th>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {visibleProducts.length === 0 ? (
                  <tr>
                    <td
                      colSpan={8}
                      className="px-4 py-12 text-center text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-900"
                    >
                  ...
                  applySort(visibleProducts).map((row, idx) => (
                    ...
                    >
                      <td className="px-4 py-2 font-medium text-slate-800 dark:text-slate-200 border-x border-slate-100 dark:border-slate-800">
                        {row.mercado}
                      </td>
                      <td className="px-4 py-2 text-slate-500 dark:text-slate-400 font-mono text-xs border-r border-slate-100 dark:border-slate-800">
                        {row.codigo}
                      </td>
                      <td
                        className="px-4 py-2 text-slate-700 dark:text-slate-300 truncate border-r border-slate-100 dark:border-slate-800"
                        title={row.produto}
                      >
                        {row.produto}
                      </td>
                      ...
                      <td className="px-4 py-2 text-right font-semibold text-orange-600 dark:text-orange-400 bg-slate-50 dark:bg-slate-800/30 border-r border-slate-100 dark:border-slate-800">
                        {formatCurrency(row.precoSugerido)}
                      </td>
                      <td className="px-4 py-2 text-right text-slate-600 dark:text-slate-400 border-r border-slate-100 dark:border-slate-800">
                        {row.precoSugerido > 0 ? (
                          <span
          ...
          <div className="p-4 border-t border-slate-200 dark:border-slate-800 text-xs text-slate-500 dark:text-slate-400 flex justify-between bg-white dark:bg-slate-900 text-center rounded-b-xl">
            <span>
              * <strong>Preço Sugerido</strong> = Custo / (1 - (27% + 15%))
              arredondado para próximo .x9
            </span>
            <span>
              <strong>Margem</strong> = 1 - (Custo / Sugerido) - % Op
            </span>
          </div>
        </section>
      )}
