      ...
      <div className="bg-white dark:bg-slate-900 p-4 border border-slate-200 dark:border-slate-800 shadow-xl rounded-xl text-sm min-w-[200px]">
        ...
        <div className="space-y-1 text-slate-600 dark:text-slate-400">
          ...
          <p>
            ...
            {data.margin?.toFixed(2) ?? "N/A"}%
          </p>
          <p>
            <span className="font-medium text-slate-700 dark:text-slate-300">
              Vendas PIX:
            </span>{" "}
            {data.pixPercent?.toFixed(2) ?? "N/A"}%
          </p>
        </div>
        <div className="mt-3 pt-2 border-t dark:border-slate-800">
          <span
            className={cn(
              ...
              data.status === "Alerta de Risco" &&
                "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400",
              data.status === "Motor da Loja" &&
                "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400",
              data.status === "Cauda Longa" &&
                "bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-400",
              data.status === "Venda Monopolizada Menor" &&
      ...
      <div className="bg-white dark:bg-slate-900 p-4 border border-slate-200 dark:border-slate-800 shadow-xl rounded-xl text-sm min-w-[220px]">
        ...
        <div className="space-y-2 text-slate-600 dark:text-slate-400">
          ...
          <p className="flex justify-between">
            ...
            <span className="dark:text-slate-200">
              {formatCurrency(data.margemBruta)}
            </span>
          </p>
          <p className="flex justify-between">
            <span className="font-medium text-slate-700 dark:text-slate-300 flex items-center">
              <span className="w-3 h-3 rounded-full bg-orange-800 mr-2"></span>
              Margem Líquida:
            </span>
            <span
              className={cn(
          ...
          <p className="flex justify-between">
            ...
            <span className="dark:text-slate-200">
              {data.transactions > 0
                ? formatCurrency(data.faturamento / data.transactions)
                : formatCurrency(0)}
            </span>
          </p>
        </div>
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white p-4 border border-slate-200 shadow-xl rounded-xl text-sm min-w-[200px]">
        <p className="font-bold text-slate-800 mb-2 border-b pb-2">
          {data.name}
        </p>
        <div className="space-y-1 text-slate-600">
          ...
          <p>
            <span className="font-medium text-slate-700">
              Margem Líquida (% do Faturamento):
            </span>{" "}
            {data.marginPercent?.toFixed(2)}%
          </p>
          <p>
            <span className="font-medium text-slate-700">Volume Absoluto:</span>{" "}
            {data.volume} unid.
          </p>
          <p>
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white p-4 border border-slate-200 shadow-xl rounded-xl text-sm min-w-[200px]">
        ...
        <div className="space-y-1 text-slate-600">
          <p>
            <span className="font-medium text-slate-700">Volume Relativo:</span>{" "}
            {data.volumePercent?.toFixed(2)}%
          </p>
          <p>
            <span className="font-medium text-slate-700">Margem Unitária:</span>{" "}
            {formatCurrency(data.margemUnitaria || 0)}
          </p>
          <p>
