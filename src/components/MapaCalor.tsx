...
import { Settings, Eye, MousePointer2, Plus, X, Check, Save, Edit2, ListPlus } from 'lucide-react';
...
export function MapaCalor({ rawData, availableUnits }: MapaCalorProps) {
  const [selectedMarket, setSelectedMarket] = useState<string>(availableUnits[0] || '');
  const [configs, setConfigs] = useState<Record<string, MarketShelfConfig>>({});
  
  const [mode, setMode] = useState<Mode>('setup');
  const [metric, setMetric] = useState<'volume' | 'faturamento' | 'margem'>('volume');
  ...
  const [manualVPos, setManualVPos] = useState<'full' | 'top' | 'bottom'>('full');

  // Drawing state
  const [isDrawing, setIsDrawing] = useState(false);
  const [drawStart, setDrawStart] = useState<{ x: number, y: number, nichoId: string } | null>(null);
  const [currentRect, setCurrentRect] = useState<Partial<Rect> | null>(null);
  const [drawingNichoRef, setDrawingNichoRef] = useState<HTMLElement | null>(null);
  const [hoveredRectInfo, setHoveredRectInfo] = useState<{ rect: Rect, x: number, y: number } | null>(null);
  ...
  const productStats = useMemo(() => {
    const stats: Record<string, { volume: number, faturamento: number, margem: number }> = {};
  ...
  const handleMouseDown = (e: React.MouseEvent, nichoId: string) => {
    ...
    const existingRects = currentConfig.rects.filter(r => r.nichoId === nichoId);
    const hSnaps = [0, 100, ...existingRects.flatMap(r => [r.x, r.x + r.width])];
    const vSnaps = [0, 100, ...existingRects.flatMap(r => [r.y, r.y + r.height])];
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    const existingRects = currentConfig.rects.filter(r => r.nichoId === currentRect.nichoId);
    const hSnaps = [0, 100, ...existingRects.flatMap(r => [r.x, r.x + r.width])];
    const vSnaps = [0, 100, ...existingRects.flatMap(r => [r.y, r.y + r.height])];
  ...
  const renderShelves = () => {
    ...
    return (
      ...
      >
        {cols.map((_, colIndex) => (
          <div key={colIndex} className="flex-1 flex flex-col border-4 border-slate-800 dark:border-slate-300 rounded-sm bg-slate-50 dark:bg-slate-900 min-w-[200px]">
            ...
            <div className="flex-1 flex flex-col p-2 gap-2">
              {rows.map((_, rowIndex) => {
                ...
                const rectsForNicho = currentConfig.rects.filter(r => r.nichoId === nichoId);
                ...
                return (
                  ...
                  >
                    ...
                    {rectsForNicho.map(rect => {
                      ...
                      const heatColor = mode === 'view' ? getHeatmapColor(statVal, metricBounds.min, metricBounds.max) : '#cbd5e1';
                      ...
                      return (
                        <div
                          ...
                          onMouseEnter={(e) => setHoveredRectInfo({ rect, x: e.clientX, y: e.clientY })}
                          onMouseMove={(e) => setHoveredRectInfo({ rect, x: e.clientX, y: e.clientY })}
                          ...
                          style={{
                            ...
                            width: `${rect.width}%`,
                            height: `${rect.height}%`,
                            backgroundColor: mode === 'view' ? heatColor : 'rgba(217, 119, 6, 0.2)',
                            borderColor: mode === 'view' ? '#00000033' : '#d97706',
                            color: mode === 'view' ? '#fff' : 'inherit',
                            textShadow: mode === 'view' ? '0px 1px 2px rgba(0,0,0,0.8)' : 'none'
                          }}
                        >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-sm shrink-0">
        <div>
           ...
           <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
             Configure as prateleiras e visualize as zonas mais "quentes" de venda ou lucratividade.
           </p>
        </div>
             ...
             <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
               <button
                 ...
                 className={cn(
                   ...
                   mode === 'edit' ? "bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm" : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"
                 )}
               >
               ...
               <button
                 ...
                 className={cn(
                   "px-4 py-1.5 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors",
                   mode === 'manual' ? "bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm" : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"
                 )}
               >
               ...
               <button
                 ...
                 className={cn(
                   "px-4 py-1.5 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors",
                   mode === 'view' ? "bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm" : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"
                 )}
               >
      ...
      <div className="flex-1 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col relative">
        {mode === 'setup' && (
          <div className="flex-1 flex flex-col items-center justify-center p-8 max-w-md mx-auto w-full text-center">
            ...
            <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-2">Configurar Layout Inicial</h3>
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">
              Defina a quantidade de prateleiras (colunas) e quantos nichos (linhas) cada prateleira possui neste mercado para iniciar o desenho.
            </p>
        ...
        {mode === 'edit' && configs[selectedMarket] && (
          <div className="flex-1 flex flex-col p-6 overflow-hidden">
            <div className="flex items-center gap-3 mb-4 bg-blue-50 dark:bg-blue-900/20 text-blue-800 dark:text-blue-200 p-3 rounded-xl border border-blue-200 dark:border-blue-800/50">
              ...
              <p className="text-sm font-medium">Instruções: Para registrar a posição de um produto, clique num nicho em branco abaixo, segure e arraste o mouse para formar o retângulo do espaço dele.</p>
            </div>
          ...
          <div className="flex-1 flex flex-col p-6 overflow-hidden md:flex-row gap-6">
            <div className="w-full md:w-1/3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/50 rounded-2xl p-6 flex flex-col gap-4 overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Prateleira</label>
                  <select
                    value={manualShelf}
                  ...
                  >
                    {Array.from({ length: currentConfig.columns }).map((_, i) => (
                      <option key={i} value={i + 1}>Coluna {i + 1}</option>
                    ))}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Nicho (Linha)</label>
                  <select
                    value={manualNiche}
                    onChange={(e) => setManualNiche(Number(e.target.value))}
                    className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg text-sm"
                  >
                    {Array.from({ length: currentConfig.rowsPerColumn }).map((_, i) => (
                      <option key={i} value={i + 1}>Linha {i + 1}</option>
                    ))}
                  </select>
                </div>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Divisões Horizontais (Qtd. Produtos)</label>
                <input
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Posição (esq para dir)</label>
                <select
                  value={manualHPos}
                ...
                >
                  {Array.from({ length: manualCols }).map((_, i) => (
                    <option key={i} value={i + 1}>Posição {i + 1}</option>
                  ))}
                </select>
              </div>

              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Empilhamento (Altura)</label>
                <select
                  value={manualVPos}
          ...
          <div className="flex-1 flex flex-col p-6 overflow-hidden">
             <div className="flex items-center justify-between mb-4 bg-slate-50 dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700/50">
                <div className="flex items-center gap-3">
                   <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Métrica do Mapa de Calor:</span>
                   <select
                     value={metric}
        ...
        {hoveredRectInfo && (
           ...
           >
              <div className="font-bold text-orange-400 mb-2 border-b border-white/20 pb-1">{hoveredRectInfo.rect.productName}</div>
              <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-slate-300">
                 <span>Volume:</span>
                 <span className="font-medium text-white text-right">{productStats[hoveredRectInfo.rect.productName]?.volume || 0} unid.</span>
                 
                 <span>Faturamento:</span>
                 <span className="font-medium text-white text-right">{formatCurrency(productStats[hoveredRectInfo.rect.productName]?.faturamento || 0)}</span>
                 
                 <span>Margem:</span>
                 <span className="font-medium text-white text-right">{formatCurrency(productStats[hoveredRectInfo.rect.productName]?.margem || 0)}</span>
              </div>
           </div>
        )}
          ...
          <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in">
            <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg shadow-xl overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col max-h-[80vh]">
              <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
                <h3 className="font-bold text-slate-900 dark:text-white">Vincular Produto ao Espaço</h3>
                <button 
                  onClick={() => { setShowProductModal(false); setCurrentRect(null); }}
