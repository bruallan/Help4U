...
interface ProcessedDay {
  ...
  status: 'pending' | 'checking' | 'correct' | 'incomplete' | 'empty' | 'divergent' | 'error';
  errorMessage?: string;
}

const API_BASE = (import.meta as any).env?.VITE_API_URL || '';

export default function AuditoriaVMPay({ rawData, onRefreshData }: AuditoriaVMPayProps) {
  ...
  const stats = useMemo(() => {
    ...
    const incomplete = auditDays.filter(d => d.status === 'incomplete').length;
    ...
    return { total, correct, incomplete, empty, divergent, errors, totalMissing };
  }, [auditDays]);
  ...
  const runRepairForDate = async (dateStr: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(`Falha no backend ao carregar dados do dia ${dateStr} (HTTP ${res.status})`);
      }
      ...
      // Update state
      setAuditDays(prev => prev.map(d => d.dateStr === dateStr ? {
        ...d,
  ...
  const handleStartAudit = async () => {
    ...
    try {
      ...
      for (let i = 0; i < daysList.length; i++) {
        ...
        setAuditDays(prev => prev.map(d => d.dateStr === day.dateStr ? { ...d, status: 'checking' } : d));
        addLog(`Consultando API para ${day.dateStr} (Banco possui ${day.dbCount} registros)...`);

        try {
          let page = 1;
          let totalApiCount = 0;
          let keepGoing = true;
          const startOfDay = `${day.dateStr}T00:00:00Z`;
          const endOfDay = `${day.dateStr}T23:59:59Z`;
          ...
          if (day.dbCount === 0 && totalApiCount === 0) {
            ...
            addLog(`⚠️ DIVERGÊNCIA em ${day.dateStr}: VMPay tem ${totalApiCount} mas Firestore tem apenas ${day.dbCount}!`);
          } else if (day.dbCount > totalApiCount) {
          ...
          setAuditDays(prev => prev.map(d => d.dateStr === day.dateStr ? {
            ...d,
      ...
      const daysToRepair = daysList.filter(d => d.status === 'incomplete' || d.status === 'error' || d.status === 'divergent');
      
      if (daysToRepair.length > 0) {
        addLog(`🔧 Autocorreção: Foram identificados ${daysToRepair.length} dias divergentes ou com falhas.`);
        ...
        try {
          for (const dayToFix of daysToRepair) {
            try {
              ...
              addLog(`⚠️ Falha na correção automática do dia ${dayToFix.dateStr}: ${repairErr.message}. Continuando para o próximo.`);
            }
          }
      ...
      addLog(`❌ Erro crítico durante o processamento da auditoria: ${err.message}`);
    } finally {
      setIsAuditing(false);
      setCurrentCheckingDay('');
    }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <div className="space-y-2">
            <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 block">Data de Início</label>
          ...
          <div className="space-y-2">
            <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 block">Data de Fim</label>
            <div className="relative">
              <Calendar className="absolute left-3 top-2.5 w-5 h-5 text-slate-400" />
              <input 
                type="date"
      ...
      {auditDays.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 shadow-sm">
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Total Período</p>
            <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1">{stats.total} dias</p>
            {currentCheckingDay && (
              <p className="text-xs text-orange-500 mt-1 animate-pulse">Auditando: {currentCheckingDay.split('-').reverse().slice(0, 2).join('/')}</p>
            )}
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 shadow-sm border-l-4 border-l-emerald-500">
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Consolidados</p>
            <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400 mt-1">{stats.correct} dias</p>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 shadow-sm border-l-4 border-l-amber-500">
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Incompletos (Faltando)</p>
            <p className="text-2xl font-bold text-amber-600 dark:text-amber-400 mt-1">{stats.incomplete} dias</p>
            <p className="text-xs text-amber-500 font-semibold mt-1">~{stats.totalMissing} vendas ausentes</p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 shadow-sm border-l-4 border-l-slate-400">
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Sem Movimento</p>
            <p className="text-2xl font-bold text-slate-500 mt-1">{stats.empty} dias</p>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 shadow-sm border-l-4 border-l-indigo-400">
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Divergentes/Excesso</p>
            <p className="text-2xl font-bold text-indigo-600 mt-1">{stats.divergent} dias</p>
            <p className="text-xs text-slate-400 mt-1">Banco possui mais registros</p>
          </div>
        </div>
      )}
      ...
      {auditDays.length > 0 && (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
          <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
            <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100">Relatório Detalhado por Dia</h4>
            <span className="text-xs text-slate-400">Resultados da verificação ativa</span>
          </div>
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-800/40 text-sm">
                {applySort(auditDays).map(day => {
                  ...
                  if (day.status === 'pending') {
                    ...
                    badge = <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium bg-orange-50 text-orange-600 dark:bg-orange-950/20 dark:text-orange-400 animate-pulse"><Loader2 className="w-3.5 h-3.5 animate-spin" /> Verificando...</span>;
                  } else if (day.status === 'correct') {
                    badge = <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-50 text-emerald-600 dark:bg-emerald-950/20 dark:text-emerald-400"><CheckCircle className="w-3.5 h-3.5" /> Consolidado</span>;
                  } else if (day.status === 'incomplete') {
                    const diff = (day.apiCount || 0) - day.dbCount;
                    badge = <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium bg-amber-50 text-amber-600 dark:bg-amber-950/20 dark:text-amber-400"><AlertCircle className="w-3.5 h-3.5" /> Falta {diff} {diff === 1 ? 'venda' : 'vendas'}</span>;
                  } else if (day.status === 'empty') {
                    badge = <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium bg-slate-50 text-slate-400 dark:bg-slate-800 dark:text-slate-500">Sem Movimento</span>;
                  } else if (day.status === 'divergent') {
                    badge = <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium bg-indigo-50 text-indigo-600 dark:bg-indigo-950/10 dark:text-indigo-400">Divergente</span>;
                  } else {
                    badge = <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium bg-red-50 text-red-600" title={day.errorMessage}>Falha ({day.errorMessage})</span>;
                  }

                  const formattedDate = day.dateStr.split('-').reverse().join('/');

                  return (
                    <tr key={day.dateStr} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                      <td className="p-4 font-medium text-slate-700 dark:text-slate-300">{formattedDate}</td>
                      <td className="p-4 text-slate-500 dark:text-slate-400">{day.dbCount} registros</td>
                      <td className="p-4 text-slate-700 dark:text-slate-300 font-semibold">{day.apiCount !== null ? `${day.apiCount} registros` : '-'}</td>
                      <td className="p-4">{badge}</td>
                      <td className="p-4 text-right">
                        <button
                          onClick={() => handleRepairDay(day.dateStr)}
                          disabled={day.status === 'correct' || day.status === 'empty' || isRepairing || isAuditing}
      ...
      <div className="bg-black/90 dark:bg-black rounded-2xl p-5 border border-slate-800 font-mono text-xs text-green-400 shadow-lg space-y-2">
        ...
        <div className="max-h-36 overflow-y-auto flex flex-col-reverse gap-1 pr-1">
          {logs.length === 0 ? (
            <span className="text-slate-600 font-sans italic">Nenhuma atividade executada ainda. Pressione "Iniciar Auditoria" para analisar o histórico.</span>
          ) : (
