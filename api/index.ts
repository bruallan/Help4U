...
app.get('/api/proxy/cashless_facts', async (req, res) => {
  try {
    ...
    const url = `${BASE_URL}/api/v1/cashless_transactions?access_token=${ACCESS_TOKEN}&start_date=${start_date}&end_date=${end_date}&per_page=50&page=${page || 1}`;
...
app.post('/api/sync-single-day', async (req, res) => {
  try {
    ...
    while (hasMore) {
      const url = `${BASE_URL}/api/v1/cashless_transactions?access_token=${ACCESS_TOKEN}&start_date=${start_date}&end_date=${end_date}&per_page=50&page=${page}`;
      const fetchRes = await fetch(url, {
        headers: {
